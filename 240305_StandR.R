...
colData(spe)$regions <- paste0(colData(spe)$Sex,"_",colData(spe)$KO,"_",colData(spe)$Diet)
...
plotSampleInfo(spe, column2plot = c("Strain","SlideName","Sex","KO","Diet"))
dev.off()
...
pdf( pdfName, width = 8 , height = 4 )
plotGeneQC(spe, ordannots = "regions", col = regions, point_size = 2, bin_num = 30, top_n=9)
dev.off()
...
spe <- spe[,rownames(colData(spe))[colData(spe)$lib_size > 1000000]]
...
plotRLExpr(spe, ordannots = "Diet", assay = 2, col = SlideName)
plotRLExpr(spe, ordannots = "KO", assay = 2, col = SlideName)
dev.off()
...
drawPCA(spe, assay = 2, col = Diet, shape = regions)
drawPCA(spe, assay = 2, col = KO, shape = regions)
dev.off()
...
pdfName <- 'PCA_series_KO-Diet-Sex_spe.pdf'
pdf( pdfName, width = 8 , height = 4 )
...
for(i in seq(20)){
  ...
  print(plotPairPCA(spe_ruv, assay = 2, n_dimension = 4, color = Diet, title = paste0("k = ", i)))
  print(plotPairPCA(spe_ruv, assay = 2, n_dimension = 4, color = KO, title = paste0("k = ", i)))
  print(plotPairPCA(spe_ruv, assay = 2, n_dimension = 4, color = Sex, title = paste0("k = ", i)))
